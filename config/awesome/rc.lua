--[[
     ...
     Base taken from copycat-killer/multicolor and fit to my liking.
     Most of the things are pretty good by default, but I don't really use a lot of things.
     Most noticeable changes:
     - Using my own theme
        * Every color is taken from the theme
     - Different layouts
     - Status bar at bottom right
     - Display layout names instead of images

     Smaller things include tag names, keybindings, etc.; the usual things.
...
-- beautiful init
beautiful.init(os.getenv("HOME") .. "/.dotfiles/themes/current/theme.lua")
-- common
modkey     = "Mod4"
altkey     = "Mod1"
...
volumewidget = lain.widgets.alsa({
    ...
    settings = function()
        ...
        widget:set_markup(markup(beautiful.colors.color4, volume_now.level .. "% "))
    end
})
...
for s = 1, screen.count() do
    ...
    mylayoutbox[s].width = 100
    mylayoutbox[s]:set_text(awful.layout.getname(awful.layout.get(s)))
    ...
    local layoutbox_constraint = wibox.layout.constraint()
    layoutbox_constraint:set_widget(mylayoutbox[s])
    layoutbox_constraint:set_strategy('exact')
    layoutbox_constraint:set_width(75)
    bottom_right_layout:add(layoutbox_constraint)

    bottom_center_layout = wibox.layout.fixed.horizontal()
    bottom_center_layout:add(mytasklist[s])

    -- Now bring it all together (with the tasklist in the middle)
    bottom_layout = wibox.layout.align.horizontal()
    bottom_layout:set_left(bottom_left_layout)
    bottom_layout:set_middle(bottom_center_layout)
